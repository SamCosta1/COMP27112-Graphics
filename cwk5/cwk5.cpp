...
unsigned char * edge_detect(unsigned char *image, int width, 
    ...
    for (int i = width; i < (width-1) * height; i++) {

        if (isLeftMostColumn(i, width, height) || isRightMostColumn(i,width,height))
            continue;
...
int main(int argc, char *argv[])
{
  ...
  unsigned char *medianImage = median(image, width,height);
  ...
  write_JPEG_file(argv[4], width, height, channels, medianImage, 95);
